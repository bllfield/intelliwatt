...
function parseGreenButtonBuffer(buffer, filename) {
  const warnings = [];
  const errors = [];
  ...
  try {
    ...
    errors.push(`Failed to parse file: ${(error === null || error === void 0 ? void 0 : error.message) || error}`);
  }
...
function parseGreenButtonXml(xml) {
...
function readingFromNode(node) {
  ...
  if (intervalNode) {
    ...
    const intervalValue = value || getScalar(node.readingValue) || getScalar(node.amount);

    if (intervalStart && intervalValue) {
      const intervalUnit = getScalar(intervalNode["@_uom"]) || getScalar(intervalNode.uom) || getScalar(intervalNode.unit);
      return {
...
function findFirstScalar(node, key) {
  if (!node) return null;
  if (Array.isArray(node)) {
    for (const entry of node) {
  ...
  if (key in node) {
    const scalar = getScalar(node[key]);
    if (scalar !== null) {
      return scalar;
    }
  }
...
function parseGreenButtonCsv(csv, warnings) {
...
function parseGreenButtonJson(json, warnings) {
  try {
    ...
    for (const row of data) {
      ...
      const ts = getScalar(row.timestamp || row.start);
      const val = getScalar(row.value || row.kwh);
      if (!ts || !val) continue;
      const dur = parseIntOrNull(getScalar(row.durationSeconds || row.duration));
      const unit = getScalar(row.unit || row.uom);
    ...
    warnings.push(`Failed to parse JSON: ${(error === null || error === void 0 ? void 0 : error.message) || error}`);
    return [];
  }
}

function normalizeGreenButtonReadingsTo15Min(rawReadings, options) {
  const treatAsEnd = (options === null || options === void 0 ? void 0 : options.treatTimestampAsEnd) || false;
  const maxKwh = (options === null || options === void 0 ? void 0 : options.maxKwhPerInterval) ?? null;
...
function roundDownTo15Minutes(d) {
  const ms = d.getTime();
...
app.post("/upload", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    const earliest = trimmed[0] ? trimmed[0].timestamp : null;
    const latest = trimmed[trimmed.length - 1] ? trimmed[trimmed.length - 1].timestamp : null;
