...
type TdspIngestAction = "created" | "noop" | "skipped" | "error";

export type PuctTdspIngestResult = {
  tdspCode: SourceEntry["tdspCode"];
  action: TdspIngestAction;
  processed: boolean;
  effectiveStartISO?: string | null;
  versionId?: string | null;
  sourceDocSha256?: string | null;
  message?: string;
};
...
async function ingestForSource(
  entry: SourceEntry,
  args: IngestArgs,
): Promise<PuctTdspIngestResult> {
  const { tdspCode, sourceUrl } = entry;
  const { debugDate } = args;

  if (!sourceUrl) {
    const message = "sourceUrl is null (TODO fill PUCT Rate_Report URL).";
    log(tdspCode, "skip:", message);
    return {
      tdspCode,
      action: "skipped",
      processed: false,
  ...
  try {
    ...
    if (textLen === 0) {
      const message =
        "pdfToText returned empty text; skipping before parsing.";
      log(tdspCode, "ERROR:", message);
      return {
        tdspCode,
        action: "skipped",
        processed: true,
    ...
    if (!effectiveStartISO) {
      ...
      const ambiguous =
        candidatesSafe.length > 1 || parsedDate.ambiguous === true;
      const message = ambiguous
        ? "ambiguous effective dates found"
        : "no effective date found in text";
    ...
    if (tdspCode === "AEP_NORTH" || tdspCode === "AEP_CENTRAL") {
      ...
      if (!row) {
        const message =
          "could not parse North/Central columns from AEP report.";
        log(tdspCode, "skip:", message);
        return {
          tdspCode,
          action: "skipped",
    ...
    if (!single) {
      const message = "could not parse single-utility rates from report.";
      log(tdspCode, "skip:", message);
      return {
        tdspCode,
        action: "skipped",
    ...
    log(tdspCode, "error during ingest:", message);
    return {
      tdspCode,
      action: "error",
      processed: false,
...
export async function runPuctRateReportIngest(args?: {
  debugDate?: boolean;
}): Promise<{
  results: PuctTdspIngestResult[];
  summary: {
  ...
  const debugDate = args?.debugDate ?? false;
  ...
  const results: PuctTdspIngestResult[] = [];

  let processedTdspCount = 0;
  let createdVersionCount = 0;
  let noopVersionCount = 0;
  let skippedTdspCount = 0;
  let errorTdspCount = 0;

  for (const entry of rateReportEntries) {
    const res = await ingestForSource(entry, { debugDate });
    results.push(res);

    processedTdspCount += res.processed ? 1 : 0;
    if (res.action === "created") createdVersionCount += 1;
    if (res.action === "noop") noopVersionCount += 1;
    if (res.action === "skipped") skippedTdspCount += 1;
    if (res.action === "error") errorTdspCount += 1;
  }
...
async function main() {
  ...
  const { summary } = await runPuctRateReportIngest({ debugDate });
