...
// Deterministic generic shapes used only when SMT intervals are unavailable.
export function genericWeekdayShape96(): Shape96 {
...
export function genericWeekendShape96(): Shape96 {
  const bins = Array.from({ length: INTERVALS_PER_DAY }, (_, i) => {
    const hour = i / 4;
