...
export async function buildOnePathManualUsagePastSimReadResult(args: {
  userId: string;
  houseId: string;
  ...
  const emit = (event: string, extra: Record<string, unknown> = {}) => {
    logSimPipelineEvent(event, {
      ...
      source: "buildOnePathManualUsagePastSimReadResult",
      ...extra,
    });
  };
  if (!args.scenarioId) {
  ...
  const effectiveUsageInputMode =
    ...
    (manualUsageRecord.payload?.mode === "ANNUAL"
      ...
      : manualUsageRecord.payload?.mode === "MONTHLY"
        ? "MANUAL_MONTHLY"
        : null);
  const out = await readOnePathSimulatedUsageScenario({
    userId: args.userId,
    houseId: args.houseId,
...
async function buildManualUsageReadDecorations(args: {
  userId: string;
  houseId: string;
  scenarioId: string | null;
  dataset: any;
