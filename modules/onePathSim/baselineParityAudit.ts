...
export function buildOnePathBaselineParityAudit(args: {
  ...
  const lookupIntervalsCount = pickNumber(lookupSummary.intervalsCount);
  const readModelIntervalsCount = pickNumber(datasetSummary.intervalsCount);
  const lookupTotalKwh = pickNumber(lookupSummary.totalKwh);
  ...
  const intervalCountParity = lookupIntervalsCount == null || readModelIntervalsCount == null
    ? null
    : lookupIntervalsCount === readModelIntervalsCount;
  ...
  return {
    userSiteBaselineOwnerUsed: "app/api/user/usage/route.ts -> resolveIntervalsLayer ACTUAL_USAGE_INTERVALS",
    onePathBaselineOwnerUsed: "modules/onePathSim/onePathSim.ts -> buildReadOnlyIntervalBaselinePreview",
    parityStatus,
    intervalCountParity,
    totalKwhParity,
    monthlyParity,
