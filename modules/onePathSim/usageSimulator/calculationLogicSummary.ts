  ...
  switch (key) {
    ...
    case "MANUAL_MONTHLY":
      return {
        ...
        modeLabel: "Pure Manual Monthly",
        stageOnePath:
          "Use the saved manual-monthly payload as Stage 1 truth first, then pass only manual bill periods, manual totals, explicit travel ranges, weather, and profile priors into the shared simulator.",
        modeOverview:
          "This run is pure manual monthly: manual month and bill-period targets stay fixed, the shared simulator derives daily and interval detail underneath them, and source actual usage remains compare-only.",
      };
    case "MONTHLY_FROM_SOURCE_INTERVALS":
    case "manual_monthly_constrained":
      return {
        modeFamily: "manual_monthly",
        modeLabel: "Monthly From Source Intervals",
        stageOnePath:
          "Run the shared manual-monthly Stage 1 helper flow first, keeping explicit source-derived monthly anchors as the monthly constraint input before the shared simulator runs.",
        modeOverview:
          "This run is source-derived monthly: source-backed monthly anchors are allowed to drive the run, bill-range semantics remain relevant for reconciliation, and daily/interval detail is generated by the shared Past simulator.",
      };
...
function monthlyLayerSummary(args: {
  modeFamily: LogicModeFamily;
  pureManualMonthlyMode: boolean;
  sourceDerivedManualMonthlyMode: boolean;
}): {
  summary: string;
  preservedOrLocked: string[];
  simulatedOrDerived: string[];
  ...
  switch (args.modeFamily) {
    case "manual_monthly":
      if (args.pureManualMonthlyMode) {
        return {
          summary: "Manual month and bill-period totals are fixed first, then the shared simulator distributes daily and interval detail underneath those manual constraints without using source intervals as Stage 2 drivers.",
          preservedOrLocked: ["Entered manual monthly totals", "Bill-range intent for later reconciliation"],
          simulatedOrDerived: ["Daily totals inside each constrained month", "Interval curves inside each simulated day"],
          modeSpecificRules: [
            "Source actual usage is compare-only for pure manual monthly.",
            "Manual monthly target diagnostics come from the saved manual payload, not source-derived monthly fitting.",
          ],
        };
      }
      return {
        summary: "Source-derived monthly anchors are fixed first, then the shared simulator distributes daily and interval detail underneath those source-backed monthly constraints.",
        preservedOrLocked: ["Source-derived monthly totals", "Bill-range intent for later reconciliation"],
        simulatedOrDerived: ["Daily totals inside each constrained month", "Interval curves inside each simulated day"],
        modeSpecificRules: [
          "Statement/bill-range semantics stay attached to the run.",
          "Monthly-target construction diagnostics explain source-derived month fitting when attached.",
        ],
      };
    case "manual_annual":
      return {
...
export function buildGapfillCalculationLogicSummary(args: {
  ...
  const pureManualMonthlyMode = selectedMode === "MANUAL_MONTHLY";
  const sourceDerivedManualMonthlyMode =
    selectedMode === "MONTHLY_FROM_SOURCE_INTERVALS" || selectedMode === "manual_monthly_constrained";
  ...
  const monthlyLayer = monthlyLayerSummary({
    modeFamily: modeInfo.modeFamily,
    pureManualMonthlyMode,
    sourceDerivedManualMonthlyMode,
  });
  ...
  const inputGroups: CalculationLogicInputGroup[] = [
    {
      ...
      evidence: [
        pureManualMonthlyMode
          ? "For pure manual monthly, source actual usage is attached for compare and diagnostics only."
    ...
    },
    {
      ...
      evidence: [
        ...
        pureManualMonthlyMode
          ? Object.keys(sourceDerivedMonthlyTotals).length > 0
            ? `Unexpected source-derived month anchors were attached for ${Object.keys(sourceDerivedMonthlyTotals).length} month(s).`
            : "No source-derived month anchors were attached, which matches pure manual monthly Stage 2 isolation."
          : Object.keys(sourceDerivedMonthlyTotals).length > 0
  ...
  const layers: CalculationLogicLayer[] = [
    ...
    },
    {
      ...
      variablesUsed: [
        ...
        pureManualMonthlyMode ? "Manual monthly diagnostics" : "Source-derived monthly/annual diagnostics",
