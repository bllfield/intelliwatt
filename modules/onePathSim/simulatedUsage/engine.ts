...
} from "@/modules/onePathSim/simulatedUsage/pastDaySimulator";
import type {
  PastDayProfileLite,
...
function countPresentSlotsForDay(
  intervals: Array<{ timestamp: string; kwh: number; homeSlot?: number | null; homeDateKey?: string | null }>,
...
export function completeActualIntervalsV1(args: {
  ...
  for (const [dk, list] of Array.from(dayIntervals.entries())) {
    if (
      ...
      countPresentSlotsForDay(list, trustedSource, dk) >= trustedIntervalThresholdForDateKey(dk, trustedSource)
    ) {
      usableDays.add(dk);
    }
  }
  ...
  for (const day of days) {
    ...
    const list = dayIntervals.get(dk) ?? [];
    const presentSlotCount = countPresentSlotsForDay(list, trustedSource, dk);
    const trustedDayThreshold = trustedIntervalThresholdForDateKey(dk, trustedSource);
    const needSimulate =
      excludedDateKeys.has(dk) ||
      (simulateIncompleteDays && presentSlotCount > 0 && presentSlotCount < trustedDayThreshold) ||
...
function applyLowDataWeatherEvidenceToProfile(args: {
  ...
  weatherEvidenceSummary?: import("@/modules/onePathSim/simulatedUsage/pastDaySimulatorTypes").PastLowDataWeatherEvidenceSummary | null;
}): PastDayProfileLite {
...
export function buildPastSimulatedBaselineV1(args: {
  ...
  lowDataSyntheticContext?: {
    ...
    intradayShape96?: number[] | null;
    weekdayWeekendShape96?: { weekday?: number[] | null; weekend?: number[] | null } | null;
    weatherEvidenceSummary?: import("@/modules/onePathSim/simulatedUsage/pastDaySimulatorTypes").PastDaySimulationContext["lowDataWeatherEvidence"];
  ...
  weatherEfficiencyDerivedInput?: import("@/modules/onePathSim/weatherSensitivityShared").WeatherEfficiencyDerivedInput | null;
  ...
  for (const dayStartMs of args.canonicalDayStartsMs ?? []) {
    ...
    if (shouldSimulateDay) {
      ...
      const result = simulatePastDay(
        ...
        pastContext,
        args.homeProfile as import("@/modules/onePathSim/simulatedUsage/pastDaySimulatorTypes").PastDayHomeProfile | null,
        args.applianceProfile as import("@/modules/onePathSim/simulatedUsage/pastDaySimulatorTypes").PastDayApplianceProfile | null,
