import { findMissingHouseWeatherDateKeys, upsertHouseWeatherDays } from "@/modules/weather/repo";
import type { DayWeather, WeatherKind } from "@/modules/weather/types";
import { WEATHER_STUB_SOURCE, WEATHER_STUB_VERSION } from "@/modules/weather/types";

const YYYY_MM_DD = /^\d{4}-\d{2}-\d{2}$/;

function clamp(n: number, lo: number, hi: number): number {
  return Math.max(lo, Math.min(hi, n));
}

function dayOfYearUtc(dateKey: string): number {
  const d = new Date(`${dateKey}T12:00:00.000Z`);
  if (!Number.isFinite(d.getTime())) return 1;
  const start = Date.UTC(d.getUTCFullYear(), 0, 1, 12, 0, 0, 0);
  return Math.max(1, Math.floor((d.getTime() - start) / (24 * 60 * 60 * 1000)) + 1);
}

function hashStringToUnit(s: string): number {
  let h = 2166136261;
  for (let i = 0; i < s.length; i++) {
    h ^= s.charCodeAt(i);
    h = Math.imul(h, 16777619);
  }
  return ((h >>> 0) % 1_000_000) / 1_000_000; // [0,1)
}

function normalMonthlyAvgF(month1: number): number {
  // Deterministic DFW-ish monthly normals (avg daily mean temp F).
  const m = clamp(month1, 1, 12);
  const normals = [48, 52, 60, 68, 75, 83, 87, 88, 81, 70, 58, 50];
  return normals[m - 1];
}

function normalMonthSwingF(month1: number): number {
  // Daily min/max spread approximation by month.
  const m = clamp(month1, 1, 12);
  const swings = [12, 12, 13, 14, 14, 13, 12, 12, 13, 13, 12, 12];
  return swings[m - 1];
}

function buildNormalAvgDay(dateKey: string): Pick<DayWeather, "tAvgF" | "tMinF" | "tMaxF" | "hdd65" | "cdd65"> {
  const d = new Date(`${dateKey}T12:00:00.000Z`);
  const month1 = Number.isFinite(d.getTime()) ? d.getUTCMonth() + 1 : 1;
  const doy = dayOfYearUtc(dateKey);
  const seasonal = Math.sin((2 * Math.PI * (doy - 1)) / 365); // annual smooth variation
  const avg = normalMonthlyAvgF(month1) + seasonal * 1.2;
  const swing = normalMonthSwingF(month1);
  const tMinF = round2(avg - swing / 2);
  const tMaxF = round2(avg + swing / 2);
  const tAvgF = round2((tMinF + tMaxF) / 2);
  const hdd65 = round2(Math.max(0, 65 - tAvgF));
  const cdd65 = round2(Math.max(0, tAvgF - 65));
  return { tAvgF, tMinF, tMaxF, hdd65, cdd65 };
}

function buildActualLastYearDay(dateKey: string): Pick<DayWeather, "tAvgF" | "tMinF" | "tMaxF" | "hdd65" | "cdd65"> {
  const normal = buildNormalAvgDay(dateKey);
  const doy = dayOfYearUtc(dateKey);
  const u = hashStringToUnit(`ACTUAL_LAST_YEAR:${dateKey}`);
  const perturb = Math.sin((2 * Math.PI * (doy - 1)) / 29) * 2.2 + (u - 0.5) * 2.6; // ~[-3.5,3.5]
  const tAvgF = round2(normal.tAvgF + perturb);
  const baseSwing = Math.max(8, normal.tMaxF - normal.tMinF);
  const swingAdjust = (hashStringToUnit(`SWING:${dateKey}`) - 0.5) * 2.0;
  const swing = clamp(baseSwing + swingAdjust, 7, 20);
  const tMinF = round2(tAvgF - swing / 2);
  const tMaxF = round2(tAvgF + swing / 2);
  const hdd65 = round2(Math.max(0, 65 - tAvgF));
  const cdd65 = round2(Math.max(0, tAvgF - 65));
  return { tAvgF, tMinF, tMaxF, hdd65, cdd65 };
}

function round2(n: number): number {
  return Math.round((Number(n) || 0) * 100) / 100;
}

function buildRow(args: {
  houseId: string;
  dateKey: string;
  kind: WeatherKind;
  version: number;
}): DayWeather {
  const payload =
    args.kind === "ACTUAL_LAST_YEAR"
      ? buildActualLastYearDay(args.dateKey)
      : buildNormalAvgDay(args.dateKey);
  return {
    houseId: args.houseId,
    dateKey: args.dateKey,
    kind: args.kind,
    version: args.version,
    tAvgF: payload.tAvgF,
    tMinF: payload.tMinF,
    tMaxF: payload.tMaxF,
    hdd65: payload.hdd65,
    cdd65: payload.cdd65,
    source: WEATHER_STUB_SOURCE,
  };
}

function uniqDateKeys(dateKeys: string[]): string[] {
  const seen = new Set<string>();
  const out: string[] = [];
  for (const raw of dateKeys ?? []) {
    const k = String(raw ?? "").slice(0, 10);
    if (!YYYY_MM_DD.test(k) || seen.has(k)) continue;
    seen.add(k);
    out.push(k);
  }
  return out;
}

export async function ensureHouseWeatherStubbed(args: {
  houseId: string;
  dateKeys: string[];
  version?: number;
}): Promise<void> {
  const houseId = String(args.houseId ?? "");
  const dateKeys = uniqDateKeys(args.dateKeys ?? []);
  const version = Number.isFinite(Number(args.version)) ? Number(args.version) : WEATHER_STUB_VERSION;
  if (!houseId || dateKeys.length <= 0) return;

  const kinds: WeatherKind[] = ["ACTUAL_LAST_YEAR", "NORMAL_AVG"];
  for (const kind of kinds) {
    const missing = await findMissingHouseWeatherDateKeys({
      houseId,
      dateKeys,
      kind,
      version,
    });
    if (missing.length <= 0) continue;
    const rows = missing.map((dateKey) =>
      buildRow({
        houseId,
        dateKey,
        kind,
        version,
      })
    );
    await upsertHouseWeatherDays({ rows });
  }
}
