...
describe("actual usage source preference", () => {
  ...
  it("returns null when GREEN_BUTTON is requested but no Green Button anchor exists (no SMT fallback)", async () => {
  ...
  it("returns null when SMT is requested but no SMT anchor exists (no Green Button fallback)", async () => {

    smtFindFirst.mockResolvedValue(null);
