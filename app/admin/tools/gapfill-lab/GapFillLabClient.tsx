...
type ApiResponse =
  | { ok: true; house: HouseOption; houses: HouseOption[]; maskedIntervals: number; metrics: any; byMonth: any[]; byHour: any[]; byDayType: any[]; worstDays: any[]; pasteSummary: string; message?: string }
      ...
      {result && result.ok && (
        <div className="space-y-6">
          <div className="p-4 rounded bg-brand-blue/5 border border-brand-blue/20">
            <div className="font-semibold text-brand-navy">Results</div>
            <div className="text-sm text-brand-navy/80 mt-1">
              {result.house?.label} · {result.maskedIntervals} masked intervals
              {result.metrics ? ` · MAE ${result.metrics.mae} kWh · RMSE ${result.metrics.rmse} · MAPE ${result.metrics.mape}%` : ""}
            </div>
          </div>
          ...
          {result.metrics && (
            <>
              ...
              {result.worstDays?.length > 0 && (
                <div>
                  <div className="font-semibold text-brand-navy mb-2">Worst 10 days (by abs error)</div>
              ...
              {result.pasteSummary && (
                <div>
                  <div className="font-semibold text-brand-navy mb-2">Paste summary</div>
                  <textarea
                    readOnly
                    value={result.pasteSummary}
                    className="w-full border border-brand-blue/20 rounded p-3 font-mono text-sm h-24"
                  />
                  <button
                    type="button"
