...
export async function POST(request: Request) {
  try {
    ...
    const uploadRecord = await prisma.greenButtonUpload.create({
      data: {
        houseId: house.id,
        utilityName,
        accountNumber,
...
const MAX_UPLOAD_BYTES = 5 * 1024 * 1024; // 5 MB safety limit for App Router
...
export async function POST(request: Request) {
  try {
    ...
    const homeIdRaw = form.get("homeId");
    const utilityNameRaw = form.get("utilityName");
    const accountNumberRaw = form.get("accountNumber");

    if (!file || !(file instanceof File)) {
      return NextResponse.json({ ok: false, error: "Missing file" }, { status: 400 });
    }
    if (!homeIdRaw || typeof homeIdRaw !== "string") {
      return NextResponse.json({ ok: false, error: "Missing homeId" }, { status: 400 });
    }

    const homeId = homeIdRaw.trim();
    if (!homeId) {
      return NextResponse.json({ ok: false, error: "Invalid homeId" }, { status: 400 });
    }
    ...
    const rawRecord = await (usagePrisma as any).rawGreenButton.create({
      data: {
        homeId: house.id,
        userId: user.id,
        utilityName,
