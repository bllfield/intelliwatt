  ...
  try {
    ...
    const uint8: Uint8Array =
      pdfBytes instanceof Uint8Array
        ? pdfBytes
        : new Uint8Array(pdfBytes as ArrayBufferLike);

    // Disable workers completely in the serverless/Node runtime.
    const anyPdfjs = pdfjsLib as any;
    if (anyPdfjs.GlobalWorkerOptions) {
      anyPdfjs.GlobalWorkerOptions.workerSrc = undefined;
    }
    anyPdfjs.disableWorker = true;
