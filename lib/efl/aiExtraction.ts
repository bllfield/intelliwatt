...
export function buildPlanRulesExtractionPrompt(
  ...
  return [
    ...
    "- Example: if the EFL says \"Night Hours = 9:00 PM – 7:00 AM\" and energy is free",
    "  during those hours on all days of the week, you MUST:",
    '  - Set planRules.rateType = \"TIME_OF_USE\".',
    '  - Add a timeOfUsePeriod with:',
    '      label: \"Free Nights\" (or the EFL label),',
    "      startHour: 21,",
    "      endHour: 7,",
    "      daysOfWeek: [0,1,2,3,4,5,6],",
    "      months: null (unless the EFL limits it to specific months),",
    "      isFree: true,",
    "      rateCentsPerKwh: null.",
    "- For non-free periods, use rateCentsPerKwh (a numeric cents/kWh value) and isFree = false.",
    "- If the EFL describes a VARIABLE plan, set rateType = \"VARIABLE\",",
    "  populate currentBillEnergyRateCents (if present) and variableIndexType when explicitly named.",
    "- For kWh tiered plans (e.g., 0–1000 kWh at X¢, >1000 kWh at Y¢), express",
    "  each band in usageTiers with the correct minKwh, maxKwh, and rateCentsPerKwh.",
    "- Do NOT flatten tiered pricing into a single average rate; leave usageTiers",
    "  as-is, even if the billing engine currently falls back to a headline rate.",
    "- For usage-based bill credits (e.g., \"$125 Usage Credit for usage ≥ 1000 kWh\"),",
    "  you MUST add a billCredits entry with:",
    '    label: a clear description (e.g. \"Usage Credit ≥1000 kWh\"),',
    "    creditDollars: the monthly credit amount in dollars (positive),",
    "    thresholdKwh: the minimum kWh required (e.g. 1000),",
    '    type: \"USAGE_THRESHOLD\".',
    '- For behavioral credits (e.g., \"AutoPay & Paperless Credit: $5/month\"),',
    "  you MUST add a billCredits entry with:",
    '    label: a clear description (e.g. \"AutoPay & Paperless Credit\"),',
    "    creditDollars: the monthly credit amount in dollars (positive),",
    "    thresholdKwh: 0 or null (no usage requirement),",
    '    type: \"BEHAVIOR\".',
    "- Always set billCredits.label to a human-readable phrase derived from the EFL text.",
