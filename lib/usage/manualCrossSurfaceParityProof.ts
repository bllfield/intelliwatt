...
export const MANUAL_CROSS_SURFACE_PROOF_VERSION = "manual_cross_surface_parity_v1";
...
export function hashManualPayloadFields(payload: ManualUsagePayload | null | undefined): {
  ...
  const statementRanges =
    payload.mode === "MONTHLY"
      ? normalizeStatementRanges(Array.isArray(payload.statementRanges) ? payload.statementRanges : [], payload)
      : [];
  const travelRanges = normalizeTravelRanges(Array.isArray(payload.travelRanges) ? payload.travelRanges : []);
  const anchorDate =
    payload.mode === "MONTHLY"
...
export function aggregateManualCrossSurfaceProofViolations(args: {
  ...
  const samePayloadAnchors = comparable.filter((leg) => samePayloadAnchorIds.includes(leg.legId));
  ...
  if (gapfillManualLeg && anchorReference?.normalizedPayloadHash) {
    if (gapfillManualLeg.normalizedPayloadHash === anchorReference.normalizedPayloadHash) {
      ...
      warnings.push(
        `gapfill_manual_monthly payload (${gapfillManualLeg.gapfillDerivedPayloadHash ?? gapfillManualLeg.normalizedPayloadHash}) differs from user-entered anchor (${anchorReference.normalizedPayloadHash}) — allowed when not intentionally same payload`
      );
    }
  }
  ...
  for (const leg of samePayloadAnchors) {
    ...
    const group = samePayloadHashGroups.get(leg.normalizedPayloadHash) ?? [];
    group.push(leg);
    samePayloadHashGroups.set(leg.normalizedPayloadHash, group);
  }
  for (const [payloadHash, group] of samePayloadHashGroups) {
    if (group.length < 2) continue;
    const uniqueDaily = new Set(group.map((leg) => leg.finalizedDailyRowsHash).filter(Boolean));
    if (uniqueDaily.size > 1) {
      violations.push(
        `same_payload_parity: finalizedDailyRowsHash differs for normalizedPayloadHash ${payloadHash} (${group
    ...
    if (uniqueMonthly.size > 1) {
      violations.push(
        `same_payload_parity: monthlyRowsHash differs for normalizedPayloadHash ${payloadHash}`
      );
    }
    const uniqueDisplayTruth = new Set(group.map((leg) => leg.displayTruthRevision).filter(Boolean));
    if (uniqueDisplayTruth.size > 1) {
      violations.push(
        `same_payload_parity: displayTruthRevision differs for normalizedPayloadHash ${payloadHash}`
      );
    }
  }

  for (const leg of comparable.filter(
  ...
  return { violations: Array.from(new Set(violations)), warnings: Array.from(new Set(warnings)) };
}
