...
- **Weather sensitivity phase rule:** Phase 1 is display + shared derived-input attachment only. The system computes and surfaces the score, attaches `weatherEfficiencyDerivedInput` before simulation execution, and marks `derivedInputAttached: true` / `simulationActive: false`. Phase 2 may later let simulation math consume that same attached derived input after validation.
