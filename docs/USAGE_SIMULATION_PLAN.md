...
- **Serverless memory (lab_validation)**: `simulatePastUsageDataset` clears the engine patched-interval buffer after `buildCurveFromPatchedIntervals` (the curve owns a copy) and omits returning `stitchedCurve` for `buildPathKind: "lab_validation"` because `dataset.series.intervals15` already materializes the same stitched series. The Gap-Fill compare route clears the full-window preloaded actual array after extracting the bounded test-day slice for join/scoring.
