...
6. **Diagnostics boundary**  
   Gap-Fill trace/debug views are artifact readers. Heavy readers remain snapshot/read-only. There is no authoritative `compareFreshMode`, no Gap-Fill-owned fresh selected-days/full-window compare execution, and no second compare engine. Shared recalc may still be launched before the read, but compare truth itself remains persisted-artifact-only. Optional user-pipeline parity may be requested for diagnostics, but it is additive/read-only and must never replace persisted compare truth.
...
- GapFill may run additional parity analytics, but those analytics are optional diagnostics only, are not a second compare truth source, and must not override artifact-backed compare rows.
