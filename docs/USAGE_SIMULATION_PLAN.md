...
- **User-facing SMT backfill gate (shipped PC-2026-12):** SMT pull/heal/refresh on user paths runs only for SMT-committed homes (`lib/usage/smtBackfillEligibility.ts`). Manual monthly Past recalc must not trigger interval validation backfill or user-facing SMT orchestration. **Master plan:** `docs/PROJECT_PLAN.md` → PC-2026-12.
...
- **One Path dedicated test-home rule:** One Path Admin writes only to a reusable `ONE_PATH_LAB_TEST_HOME`. The selected source house stays read-only. Replacing the test home resets isolated mutable state, syncs missing home/appliance profiles, and clones Green Button actual-usage records so admin tuning stays sandboxed.
- **One Path actual-source rule:** on a pinned One Path test home, actual usage still comes from the shared usage layer. SMT interval truth may use the selected source house ESIID as the actual meter identity fallback for the pinned actual-context house, while Green Button truth is read from persisted Green Button usage cloned onto the test home.
- **Green Button shifted-actual rule:** shifted Green Button actuals are part of the shared actual-usage layer and must be handled by the adapter before simulation. A shifted source day with trusted coverage (90 intervals, bounded by `expectedIntervalsForDateISO()` for DST) remains actual-backed for the target date, wins over a trailing/current-year partial day for that same target date, preserves `sourceDateByTargetDate`, discloses source-day weather use, and must not be classified downstream as `SIMULATED_INCOMPLETE_METER` unless the shifted source day itself is below trusted coverage.
- **One Path coverage-window rule:** for non-baseline runs, One Path coverage metadata must stay owned by `resolveCanonicalUsage365CoverageWindow()` / shared metadata-window helpers. `dataset.summary.start/end`, `dataset.meta.coverageStart/coverageEnd`, and bounded `excludedDateKeys*` metadata must stay aligned to that same shared window.
- **One Path best-effort SMT recovery rule:** interval runs may request SMT refresh and wait for canonical tail coverage before the first run, but stale/incomplete tail no longer blocks display. If the first result still contains `SIMULATED_INCOMPLETE_METER` days, the route refreshes those exact dates, rebuilds the engine input, reruns once, and still returns results if some incomplete simulated days remain.
- **Known-house sandbox rule:** repeated One Path tuning runs should use a sandbox-only, code-backed known-house scenario registry/preset loader. It may preload keeper-user identity, house/context selection, scenario selection, validation inputs, travel ranges, and review expectations into the existing admin harness controls, but it must not add live persistence or touch live consumer surfaces.
...
- **Gap-Fill target pool semantics (product intent)**: The **reference / good-data pool** for the shared Past sim **excludes** travel/vacant (and similar “bad at-home” exclusions). **Validation holdout (PC-2026-10, production Past):** validation/test scored days are **excluded** from donor/shape pools when simulating those same days (`validationHoldoutDateKeysLocal`, `strict_holdout`) — their actuals may still inform **other** days’ pools under leave-one-out semantics, but not their own holdout sim. **Gap-Fill lab** bounded test-day keep-ref is separate (see `docs/PAST_VALIDATION_HOLDOUT.md`). Travel/vacant days **do not** contribute as reference; they are **filled** by simulation. **Simulated output** used to **grade** validation days must be holdout sim from the shared module—not meter presented as “sim.”
- **Global compare-day selection (MG-2):** Which calendar dates become `validationOnlyDateKeysLocal` before holdout wiring. Single owner `lib/usage/validationDayPolicy.ts`; admin control `/admin/tools/validation-day-policy`; wired on One Path, Manual GapFill, GapFill Lab (except source-copy parity). **Contract:** `docs/GLOBAL_COMPARE_DAY_POLICY.md`. Separate from holdout scoring (`docs/PAST_VALIDATION_HOLDOUT.md`).
- **Past stitch vs compare (user Past / shared artifact reads):** The producer records **canonical simulated-day totals** for validation holdout (`VALIDATION_HOLDOUT`) and TRAVEL_VACANT modeled days; pre-projection daily rows may tag `SIMULATED_TEST_DAY` vs `SIMULATED_TRAVEL_VACANT`. **User-facing stitched Past** applies **`projectBaselineFromCanonicalDataset`** so **validation-only** dates show **meter-backed ACTUAL** (`ACTUAL_VALIDATION_TEST_DAY`); **TRAVEL_VACANT** stays simulated. **Compare** uses holdout sim totals only; UI metric is **Reconstruction check** until `meta.validationHoldoutProof.ok`, then **Holdout WAPE** (`docs/PAST_VALIDATION_HOLDOUT.md`). **`rehydrateValidationCompareMetaFromBuildInputsForRead`** restores `validationOnlyDateKeysLocal` when cache JSON dropped keys. **`attachValidationCompareProjection`** reads canonical simulated totals from meta—no second simulator. For validation days with actual totals, **`series.intervals15`** is **scaled** to match meter daily total so 15‑minute stitch energy matches the displayed actual day.
...
- **Pure manual GapFill rule:** GapFill `MANUAL_MONTHLY` is the pure manual monthly calibration path on an interval-backed house. Once Stage 1 is locked, Stage 2 may use only manual bill periods/totals, explicit travel ranges, weather, and home/appliance priors; source actual usage is compare-only.
...
- **Weather sensitivity / home efficiency shared-owner rule:** weather sensitivity scoring is one shared derived-input system only. Customer Usage, customer Simulation, admin GapFill, admin Usage-style diagnostics, the dedicated Admin Tools lab, and future simulation consumers must all read the same shared owner/output contract. No route-local, UI-local, or lab-only scoring formulas are allowed.
- **Shared simulation variable policy rule:** meaningful shared tuning coefficients and thresholds must live in the shared simulation variable policy store, not as page-local or route-local constants. Family config stays mode-aware through shared buckets only (`sharedDefaults`, `intervalOverrides`, `manualMonthlyOverrides`, `manualAnnualOverrides`, `newBuildOverrides`).
- **Effective-run variable snapshot rule:** canonical artifact/read-model output must publish `effectiveSimulationVariablesUsed` for the exact run identity. That snapshot is shared read truth, not page-assembled JSON, and must label each resolved value source (`shared default`, mode override, or explicit admin override).
- **Weather ownership rule:** weather remains under one shared owner only, with exactly two scoring paths only: `INTERVAL_BASED` and `BILLING_PERIOD_BASED`. No third path, no caller-based weather branch, and no page-local or route-local weather math are allowed.
- **Weather sensitivity calculation rule:** `weatherEfficiencyDerivedInput` belongs to the shared downstream calculation path only. It acts as a shared shape modifier, not a target setter. For `MANUAL_MONTHLY` and `MANUAL_ANNUAL`, weather-driven amplitude must remain compressed relative to interval-backed mode so bill-target / parity ownership stays authoritative.
- **Weather sensitivity fitting-pool rule:** interval-backed houses score from actual interval-backed daily usage plus same-day weather only; simulated travel/vacant days, incomplete-meter filler days, compare rows, projection rows, and manual simulated rows are excluded. Manual monthly houses score from entered bill-period totals plus weather aggregated over the exact bill dates, not from simulated daily curve output.
- **Weather sensitivity factor rule:** required up-front home/appliance factors (square footage, occupancy, fuel/HVAC/heating, thermostat settings, pool, EV, known major loads, HVAC age/SEER when available) adjust interpretation in Phase 1. Insulation/window details remain optional first-pass inputs and become the next diagnostic ask only when inefficiency or low explanatory confidence is indicated.
...
- GapFill monthly, Manual Lab monthly, user manual monthly, and One Path monthly must ultimately use the same shared manual-monthly calculation logic after normalization. No caller-specific Stage 2 sim behavior, route-local post-sim formatter, or page-local shaping difference is allowed.
...
- Manual Lab remains the reference for manual-entry Stage 1 contract resolution, but saved customer/source manual payloads remain the data truth. GapFill manual modes must therefore resolve and persist the same shared manual payload contract before recalc, and `MANUAL_TOTALS` travel ownership comes from that manual payload contract rather than a second route-local DB travel truth owner.
- One Path Admin manual modes now use that same Stage 1 helper family for unresolved admin/test-home loads: if no saved manual payload exists yet on the selected test home, admin load/run may derive monthly bill-period totals or annual totals plus anchor / statement ranges from interval-backed actual usage, but only those resolved manual totals and Stage 1 metadata may cross into Stage 2.
- For pure manual monthly, source-derived monthly anchors must not be auto-attached as active Stage 2 ownership. If source-derived monthly anchors are intentionally active, that run must stay labeled as `MONTHLY_FROM_SOURCE_INTERVALS`.
...
- Manual monthly simulation may only consume resolved month/bill-period totals. If actual usage seeds those totals, only the totals cross into Stage 2; raw intervals, source daily rows, donor-day truth, and actual intraday shapes remain compare-only.
- Manual monthly source-backed bill periods that overlap travel/vacant dates are excluded from source-truth ownership. They stay visible in Stage 1/readback diagnostics, but the shared simulation path fills them instead of reusing actual-derived totals for those periods.
- Manual annual simulation enters Stage 2 with only the annual total and anchor window. Precomputed monthly splits are not Stage 1 truth; month/day/hour distribution is derived inside the simulator path from that annual total.
...
- GapFill Actual House summary/monthly/diagnostic displays must read the same shared Past artifact/read-model truth as interval Actual House; do not let a UI-local lockbox fallback blank out values that already exist on shared diagnostics/readback.
- Actual compare totals must use that same shared actual-house truth source as the Actual House monthly display. If multiple actual totals exist, they must be explicitly labeled rather than silently mixed under one annual compare label.
- Manual compare Actual kWh must be sourced from that same shared actual-house interval-backed truth by displayed bill period, and missing actual-backed compare truth must stay null/unavailable rather than falling back to `0.00`.
...
- Travel/vacant simulated days and manual-constrained simulated days use the same shared day-simulation family and shaping logic. Their difference is reconciliation/constraint role, not a separate flat/frozen simulation path.
### Plan Change (2026-04) — Manual Monthly Canonical Actual + Active Contract Pass
- Canonical actual-house artifact truth is now the only owner for manual compare actual-reference. Compare annual totals, monthly actual totals, and bill-period Actual kWh must resolve from the same actual-house artifact/read model that drives the Actual House display, or stay explicitly unavailable if that truth is absent.
- The active GapFill `MANUAL_MONTHLY` run must write and read back the same saved/effective travel contract selected for that recalc. A stale saved payload snapshot may remain historical context, but it must not be presented as the active artifact contract when a newer effective contract was used for the run.
- Shared low-data/manual weather evidence must stay active for travel/vacant simulation too. `TRAVEL_VACANT` and `MANUAL_CONSTRAINED` remain one shared modeled-day family with shared weather/day-shape routing; only their reconciliation semantics differ.
...
- One Path Admin unresolved manual loads use that same rolling auto-date ownership for default admin bill-end / statement-range autofill.
