...
- **Manual Monthly GREEN closeout (2026-06-08):** accepted production state documented in `docs/MANUAL_MONTHLY_GREEN_CLOSEOUT.md` — authoritative shipped manual monthly handoff. Bill Match 12/12, canonical Past 2025-06-08→2026-06-07 (365 rows), manual Past source labels + estimated weather copy, Usage Simulator manual source-state copy. Manual GapFill overhaul not started. MG-0 Audit = GapFill planning context only (supersedes pre-closeout planning-green caveats). MG-1 not started.
