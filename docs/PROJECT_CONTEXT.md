...
- **Manual Monthly GREEN closeout (2026-06-08):** accepted production state documented in `docs/MANUAL_MONTHLY_GREEN_CLOSEOUT.md`. Manual GapFill full overhaul not started. **MG-1–MG-4 shipped** (`814e0839`–`be2ff2cf`). **MG-5 local:** source actual vs lab simulated compare envelope (admin diagnostic only; no Past Sim/seed writes/scoring changes). **MG-6 not started** (UI wiring). Legacy GapFill + `EXACT_INTERVALS` remain.
