...
- **Manual Monthly GREEN closeout (2026-06-08):** accepted production state documented in `docs/MANUAL_MONTHLY_GREEN_CLOSEOUT.md`. Manual GapFill full overhaul not started. **MG-1 shipped** (`814e0839` read-only source context). **MG-2 shipped** (`d9542e06` global validation-day policy/admin preview). **MG-3 local:** seed preparation from MG-1 (dry-run default; optional lab persist only; no Past Sim/compare). **MG-4/MG-5 not started** (run/readback; compare). Legacy GapFill + `EXACT_INTERVALS` remain.
