...
- **Manual Monthly GREEN closeout (2026-06-08):** accepted production state documented in `docs/MANUAL_MONTHLY_GREEN_CLOSEOUT.md`. Manual GapFill full overhaul not started. **MG-1 shipped** (`814e0839`). **MG-2 shipped** (`d9542e06`). **MG-3 shipped** (`f1159ef5` seed prep). **MG-4 local:** run/readback from prepared lab seed (canonical Past Sim on lab home; no compare). **MG-5 not started** (compare). Legacy GapFill + `EXACT_INTERVALS` remain.
