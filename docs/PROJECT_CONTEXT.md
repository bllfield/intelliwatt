...
- **Manual Monthly GREEN closeout (2026-06-08):** accepted production state documented in `docs/MANUAL_MONTHLY_GREEN_CLOSEOUT.md`. Manual GapFill full overhaul not started. **MG-1 shipped** (`814e0839` read-only source context). **MG-2 local:** global validation-day policy/admin preview. **MG-3+ not started** (seed/compare). Legacy GapFill + `EXACT_INTERVALS` remain.
