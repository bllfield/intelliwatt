...
- Manual Lab remains the reference Stage 1 contract owner for manual-entry modes. GapFill manual monthly/manual annual now resolve and persist that same shared manual payload contract before recalc, and manual payload travel ranges are the authoritative manual travel input on `MANUAL_TOTALS`.
...
- Weather sensitivity / home efficiency scoring now follows the same shared-logic rule: one shared owner computes the score, one shared derived-input stub attaches before simulation, and customer Usage, customer Simulation, admin GapFill, and the dedicated Admin Tools lab all consume that same contract. Phase 1 is display/stub only (`derivedInputAttached: true`, `simulationActive: false`); simulation outputs remain unchanged until a later validated Phase 2.
