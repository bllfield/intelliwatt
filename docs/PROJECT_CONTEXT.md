...
- **Manual Monthly GREEN closeout (2026-06-08):** accepted production state documented in `docs/MANUAL_MONTHLY_GREEN_CLOSEOUT.md`. **MG-1–MG-5 shipped** (`814e0839`–`8205fa1e`). **MG-6 (local):** admin UI at `/admin/tools/manual-gapfill` wires MG-1 source context, MG-2 validation policy, MG-3 seed, MG-4 run/readback, MG-5 compare via existing endpoints only; no new orchestration backend; legacy GapFill + `EXACT_INTERVALS` unchanged; MG-6 does not change production scoring/WAPE/Simulation Accuracy. **MG-7** = proof/docs closeout.
