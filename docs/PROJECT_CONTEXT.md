...
- **SMT interval coverage (shipped PC-2026-05):** **`docs/SMT_UNIFICATION_COMPLETE.md`** + **`docs/PROJECT_PLAN.md` → PC-2026-05**. Permanent enforcement: `.cursor/rules/smt-unification-lock.mdc`. SMT strict 96/96; single heal `lib/usage/ensureSmtCoverage.ts`; window `lib/usage/canonicalMetadataWindow.ts`.
- **User-facing SMT backfill gate (shipped PC-2026-12):** **`docs/PROJECT_PLAN.md` → PC-2026-12**. `lib/usage/smtBackfillEligibility.ts` gates SMT pull/heal/refresh to SMT-committed homes only (stored `SMT` or legacy-inferred active SMT). Green Button and manual/uncommitted homes no-op at `requestUsageRefreshForUserHouse`, user-facing `ensureSmtCoverage` profiles, and upstream seed. One Path **`admin_sim`** bypass unchanged.
- **Usage interval source of truth (shipped PC-2026-08):** **`docs/USAGE_INTERVAL_SOURCE_OF_TRUTH.md`** + **`docs/PROJECT_PLAN.md` → PC-2026-08**. Permanent enforcement: `.cursor/rules/usage-interval-ingest-lock.mdc`. Green Button: ingest only in `runGreenButtonUsagePipeline` (app + Droplet); read only via `loadPersistedGreenButtonIntervals`; stale ingest gated by `greenButtonIntervalReadiness`. SMT: ingest via `normalizeSmtIntervals`; read via `convertSmtPersistedRowsToHome` — no read-time normalize/repair. Raw vendor bytes are for re-ingest, not live Usage/Past/plan truth.
- **Past validation holdout (shipped PC-2026-10):** **`docs/PAST_VALIDATION_HOLDOUT.md`**. Production Past validation compare is true holdout (GB + SMT shared core): validation display stays ACTUAL; compare uses `VALIDATION_HOLDOUT` sim only; metric **Holdout WAPE** when `validationHoldoutProof.ok`. Gap-Fill lab keep-ref is not production validation.
- **Global compare-day policy (shipped MG-2):** **`docs/GLOBAL_COMPARE_DAY_POLICY.md`**. Single admin control plane for *which* calendar days are validation/compare days (`validationOnlyDateKeysLocal`). Owner: `lib/usage/validationDayPolicy.ts`; admin UI `/admin/tools/validation-day-policy`; persist FeatureFlag `validation_day_policy.v1`. Wired: One Path, Manual GapFill, GapFill Lab (except source-copy parity). Holdout scoring remains separate (`PAST_VALIDATION_HOLDOUT`).
...
- One Path Admin manual monthly/manual annual now mirror that admin convenience behavior too: when the selected test home has no saved manual payload, One Path may derive admin seed monthly totals, annual totals, anchor, and statement ranges from interval-backed actual usage using the same Stage 1 helper semantics, while keeping intervals and source daily rows compare-only.
...
- One Path Admin unresolved manual payloads now use that same rolling admin auto-date contract for default anchor / statement-range autofill.
...
- **Manual Monthly GREEN closeout (2026-06-08):** accepted production state in `docs/MANUAL_MONTHLY_GREEN_CLOSEOUT.md`.
- **Manual GapFill closeout (MG-1–MG-6 shipped):** authoritative admin pipeline doc `docs/MANUAL_GAPFILL_CLOSEOUT.md`. Commits `814e0839`–`ae380115`. **Primary admin workflow:** `/admin/tools/gapfill-lab` Manual GapFill Lab (MG-1–MG-5 via `ManualGapfillLabWorkflow`); `/admin/tools/manual-gapfill` is API Debug only; legacy GapFill + `EXACT_INTERVALS` remain under Advanced Legacy GapFill; no production scoring/WAPE/Simulation Accuracy changes.
...
- One Path Admin now uses a dedicated reusable `ONE_PATH_LAB_TEST_HOME` as the only writable home for manual payloads and scenario runs. Replacing it from a selected source house refreshes isolated mutable state, syncs missing home/appliance profiles, and clones Green Button actual-usage records so tuning remains sandboxed.
- On a pinned One Path test home, shared actual-usage sourcing stays mode-specific: SMT interval truth may still use the selected source house ESIID as the actual meter identity fallback for the pinned actual-context house, while Green Button truth is read from **ingest-current** persisted `GreenButtonInterval` rows cloned onto the test home (same read stack as production; `greenButtonIntervalReadiness` applies).
- Green Button shifted actuals are a generalized actual-usage rule, not a date-specific workaround. A shifted source day that meets the shared trusted threshold (90 intervals, bounded by `expectedIntervalsForDateISO()` for DST) must remain actual-backed for its target date, must win over trailing/current-year partial intervals for that target date, must preserve `sourceDateByTargetDate` and source-day weather disclosure, and must not reach Past/One Path simulation as `SIMULATED_INCOMPLETE_METER` unless the shifted source itself is below the trusted threshold.
- One Path non-baseline coverage metadata must stay on the shared canonical 365-day window from `resolveCanonicalUsage365CoverageWindow()` / shared metadata-window helpers; route code must not privately derive competing coverage windows or exclusion fingerprints.
- One Path SMT recovery is now best-effort. Interval runs may request a refresh and wait for canonical tail coverage before the first run, but stale/incomplete tail no longer blocks results with `409`. If the first result still contains `SIMULATED_INCOMPLETE_METER` days, the route refreshes those exact dates, rebuilds the engine input, and reruns once; any remaining incomplete simulated days are returned with diagnostics instead of blocking.
