...
Manual monthly and manual annual now enter the shared Past producer through shared bill-period target adapters, while new build still remains outside the full canonical shared day-level weather-driven contract. Closing the remaining fingerprint and new-build gaps remains the **final** architecture objective. **Rollout order:** Section 26 defines implementation phases; broad adapter unification follows the performance/hosting gate in Section 12.
...
| manual monthly (`MANUAL_TOTALS`) | shared monthly bill-period adapter in `buildSimulatorInputs`/service using `buildManualBillPeriodTargets()` | `WholeHomeFingerprint` (+ optional low-confidence blend) | eligible monthly bill periods are hard parity constraints; travel-touched bill periods are excluded rather than fatal | same 365-day concept; no separate weather path | required | required | shared bill-period-first adapter is now wired, but broader fingerprint persistence/blending work remains open |
| manual annual (`MANUAL_TOTALS`) | shared annual bill-period adapter in `buildSimulatorInputs`/service using the same `buildManualBillPeriodTargets()` contract | `WholeHomeFingerprint` (+ optional low-confidence blend) | derived annual bill period is a hard parity constraint when eligible; travel overlap excludes it from parity scoring | same 365-day concept; no separate weather path | required | required | shared annual bill-period adapter is now wired, but broader fingerprint persistence/blending work remains open |
...
| `manual_monthly_constrained` | `WholeHomeFingerprint` with shared monthly bill-period adapter | Eligible entered monthly bill periods are hard constraints. Travel-touched periods are excluded from parity shaping instead of failing the build. For admin lab runs on a real house, monthly constraints are derived from that house’s actual usage history for the test run unless explicit manual monthly inputs are present on the dedicated test home. | same canonical chain in Section 3 | shared `compareProjection` family only | admin-lab-only; does not alter user-facing artifacts by itself |
| `manual_annual_constrained` | `WholeHomeFingerprint` with shared annual bill-period adapter | The derived anchor-year bill period is a hard constraint when eligible. Travel overlap excludes it from parity scoring instead of failing the build. For admin lab runs on a real house, annual constraint is derived from that house’s actual usage history for the test run unless explicit manual annual input is present on the dedicated test home. | same canonical chain in Section 3 | shared `compareProjection` family only | admin-lab-only; does not alter user-facing artifacts by itself |
