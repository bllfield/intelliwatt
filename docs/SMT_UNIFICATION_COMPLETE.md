...
3. **Heal** — `ensureSmtCoverage.ts` only; targeted backfill via `smtIncompleteMeterBackfill.ts` only from ensure. In-span days use `resolveSmtHealBackfillDateKeys`; canonical tail days **after** persisted max use `resolveSmtTailExtensionHealDateKeys` (`resolveSmtHealBackfillDateKeysWithTailExtension`). Wide backfill does not request dates before persisted start; refresh wide-backfill retries sooner (30m) while heal-scope gaps remain. Optional `esiid` on `ensureSmtCoverageForHouse` covers houses with no meter row (isolated One Path lab test homes keep `esiid` null); admin One Path runs pass `resolveOnePathAdminSmtHealTarget()` so pull/backfill use the **linked source house** (ESIID + SMT authorization) while simulation artifacts stay on the pinned test home.
