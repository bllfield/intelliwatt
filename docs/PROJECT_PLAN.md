...
- **One Path manual Stage 1 publication rule:** One Path manual Stage 1 must be surfaced from One Path-owned wrappers/read models under `modules/onePathSim/**` with Manual Lab semantics as the source of truth, not from the current user manual page.
