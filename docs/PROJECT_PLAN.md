...
- **Admin tooling (single-pane rule):**
  ...
  - Added **Backfill EFL Templates** button on `/admin/efl/fact-cards` that calls `POST /api/admin/efl/backfill` using `x-admin-token` (from `localStorage.intelliwatt_admin_token` or `sessionStorage.ADMIN_TOKEN`) and shows the last response.
