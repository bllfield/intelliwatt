...
- **MG-2 shipped:** global compare-day policy — **canonical doc `docs/GLOBAL_COMPARE_DAY_POLICY.md`**. Module + preview (`d9542e06`); admin persist + wired surfaces (`5021453f`); source-copy refresh-then-continue on stale policy stamps (`ensureSourceCopyValidationPolicyFresh`). Manual GapFill, One Path, and GapFill Lab (non source-copy) consume global policy; per-run overrides removed.
...
| Doc | Role |
|-----|------|
| `docs/SMT_UNIFICATION_COMPLETE.md` | Shipped owners, closure greps, reference audit |
| `docs/SMT_UNIFICATION_PLAN.md` | Architecture, product rules, per-phase file map, session heal contract |
| `docs/SMT_UNIFICATION_PHASE_PROMPTS.md` | Historical phase prompts (pre-check → implement → post-check) |
| `docs/SMT_UNIFICATION_AGENT_BOOTSTRAP.md` | Chat bootstrap; maintenance changes must respect COMPLETE + lock rule |
| `.cursor/rules/smt-unification-lock.mdc` | **Permanent** agent constraints on SMT-related edits |
| `docs/USAGE_LAYER_MAP.md` | SMT layer owner table |
| `docs/ONE_PATH_SIM_ARCHITECTURE.md` | One Path SMT heal via ensure only |
...
| Per-day status in window | `lib/usage/smtWindowStatus.ts` (`resolveSmtCanonicalWindow` delegates here) |
...
**Forbidden on ongoing SMT work (see `.cursor/rules/smt-unification-lock.mdc`):**

- `requestTargetedSmtIntervalBackfillForHouse` outside `lib/usage/` (except tests)
...
- Production literals `canonicalCoverageLagDays: 2` / `reliableLagDays: 2` outside `lib/usage/canonicalCoverageConfig.ts` + tests
- Reimplementing `resolveCanonicalUsage365CoverageWindow` outside `lib/usage/canonicalMetadataWindow.ts`
...
5) Persisted interval artifacts:
   ...
   - `lib/usage/resolveIntervalsLayer.ts`
   - `modules/usageSimulator/pastCache.ts`
6) SMT interval coverage + persistence (PC-2026-05 shipped owners):
...
- **`run_test_home_canonical_recalc`:** After test-house variables are collected and normalized, the run uses the same shared Past Sim recalc, dataset-build, and artifact-persistence path as the normal user flow. There is no special GapFill or test-house truth path after lockbox entry.
...
- GapFill canonical compare reads persisted actual-house and test-house artifacts only. It does not simulate, rebuild, or own compare truth.
- GapFill actual-house and test-house result panels reuse the same shared Past Sim display modules as the user page for persisted outputs.
...
- Exact-interval / actual-backed modeled-day reconstruction now uses bounded K-nearest weather-similar donor blending inside the trusted reference pool as the primary daily-target selector; donor-pool variance guardrails damp noisy donor cohorts, heating-day matching weights HDD/min-temp more heavily, broad calendar ladders stay fallback-only, post-donor weather adjustment stays bounded fine-tuning, and home/appliance profiles remain secondary context in that observed-history mode.
- GapFill donor diagnostics must separate donor-path usage from true broad fallback usage, and the Actual House lockbox-flow panel must show the same shared-diagnostics truth already attached to the artifact.
...
- GapFill `MANUAL_MONTHLY` now uses one authoritative rolling auto-date Stage 1 contract: active anchor/bill-end date = current Chicago date minus 2 days. Canonical manual-monthly test-home runs must not stay pinned to an older saved bill-end day.
- Manual Usage Lab monthly popup now supports three lab-owned date-source modes: `CUSTOMER_DATES`, `AUTO_DATES`, and `ADMIN_CUSTOM_DATES`.
- `CUSTOMER_DATES` reads customer/source statement-date structure as read-only context only. Admin changes must never overwrite customer/source statement dates, totals, or travel ranges.
- `AUTO_DATES` and `ADMIN_CUSTOM_DATES` persist only on the isolated lab home / GapFill test-home payload. Admin monthly date edits must never copy back or silently synchronize into the customer/source payload.
...
- GapFill manual modes now surface a dedicated Stage 1 UI panel for admin testing on the Test Home only. That panel is additive readback from the shared manual read model; it must not replace or mutate the Actual House chart/data path.
...
- Manual monthly compare/reconciliation must enforce exact parity only on non-travel-eligible bill periods/month targets. Travel-overlapped periods remain visible but must be explicitly excluded/labeled rather than silently included in the exact-match pool.
...
- Phase‑1 plan lives in: `docs/plans/USAGE_SIMULATOR_WORKSPACES_V1_PHASE1.md`
- If anything below conflicts with that file, the plan file wins.

### Key corrections (Phase 1)

- **No simulator “Step 1: choose source” UI**. Source selection happens on Usage Entry (`/dashboard/api`). Simulator is entered via redirect intents or when partial actual coverage requires gap-fill.
- **No “ACTUAL_BASELINE mode”**. Actual baseline is the saved usage dataset served by `GET /api/user/usage` (SMT or Green Button). Simulator never modifies actual months/intervals.
- **Gap‑fill rule (critical)**: for partial SMT/GB, simulate ONLY missing months deterministically; do NOT rescale/reshape/modify actual months.
- **Current implementation shorthand note - Manual totals immutability**: this phrase refers to Stage 1 input ownership only. Preserve the user-entered bill-cycle values for the bill-cycle months the user actually entered. It does not mean the final normalized Past Sim artifact must leave missing bill-cycle months blank forever.
- **Current implementation shorthand note - Manual anchor date (billing‑cycle)**: `anchorEndDate (YYYY‑MM‑DD, America/Chicago)` defines the Stage 1 bill-cycle input chart. The anchor date is the last day of the entered bill-cycle sequence, and that sequence runs backward by bill-cycle months before the result is normalized into the shared Past Sim coverage window.
- **Current implementation shorthand note - Travel/vacant applies to user manual monthly too**: travel/vacant-aware behavior is not a GapFill-only or source-derived-only concept. User manual monthly may carry travel ranges in the manual payload, and those travel ranges must participate in shared Past Sim exclusion/simulation after normalization.
...
- ✅ System no longer returns 0% parseConfidence when obvious pricing lines exist; instead it surfaces the best-effort structured parse plus explicit fallback warnings.
 ...
 - ✅ Version extraction now also supports “split Ver.#” footers where the `Ver. #:` line contains a plan-family label (e.g. `GreenVolt`) and the unique structured code appears on the next line (e.g. `24_ONC_U_...`), plus wrapped `Version Number` labels whose identity value continues on following lines (e.g. utility fragments such as `[WTU /` then `AEP]_06162025`), stitching them into a single `eflVersionCode` for safer template identity.
...
- ✅ TDSP per-kWh extraction now continues scanning when a TDSP-context candidate line cannot actually parse a numeric charge. This prevents multi-table EFLs from stopping on demand-meter average-price context and missing the residential split-unit delivery row (e.g., `5.66770¢    ¢ per kWh`). Rule update: `R-TDSP-ROW-002`.
- ✅ TDSP per-kWh extraction now also accepts cents-only delivery rows when the line has explicit `TDU`/`TDSP Delivery Charge` context, covering PDF text extraction cases where the `¢ per kWh` unit column is separated or dropped. Rule update: `R-TDSP-ROW-002`.
- ✅ TDSP territory inference now recognizes legacy AEP utility service-area labels: WTU / West Texas Utilities maps to `AEP_NORTH`, and CPL / Central Power and Light maps to `AEP_CENTRAL`. This lets masked/pass-through TDU disclosures use the shared utility-table validation fallback instead of queueing with `tdspAppliedMode=NONE`. Rule: `R-TDSP-TERRITORY-002`.
