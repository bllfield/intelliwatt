...
- Optional user-pipeline parity may still be requested as diagnostics-only analysis over the same persisted read family, but it must stay read-only and must not replace canonical compare truth ownership.
