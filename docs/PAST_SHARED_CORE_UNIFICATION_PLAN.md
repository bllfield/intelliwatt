...
- **Interval source of truth (PC-2026-08):** Past producers and GapFill compare must load **ingest-trusted** intervals only (`docs/USAGE_INTERVAL_SOURCE_OF_TRUTH.md`). No read-time Green Button slot repair; no raw vendor XML as sim input; stale GB ingest fails closed until re-upload/rehydrate. Cached Past artifacts are snapshots — invalidate when interval fingerprint or GB `intervalIngestVersion` changes.
...
- Green Button shifted prior-year actuals are resolved before `CanonicalSimulationEngineInput` in the shared adapter. Trusted shifted source days use the shared 90-interval threshold bounded by `expectedIntervalsForDateISO()` for DST, remain actual-backed on their target date, win over trailing/current-year partial days for the same target, and carry `sourceDateByTargetDate` plus source-day weather disclosure into diagnostics/readback. They must not be converted to `SIMULATED_INCOMPLETE_METER` downstream unless the shifted source day fails trusted coverage.
